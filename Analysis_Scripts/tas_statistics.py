...
def compute_statistics(
    ...
    # Compute histogram
    counts, _ = np.histogram(temperature, bins=bins)
    region_dict[file_name][scenario][season]["counts"] = counts.tolist()

    fldmean = df["tas"].mean(dim=["rlat", "rlon"], skipna=True)
...
def calc_statistics(overwrite=False) -> None:
    ...
    for region in regions:
        output_json_file = f"tas/{region}.json"
        ...
        for file_name, scenarios in time_info.items():
            ...
            for scenario in scenarios:
                ...
                datasets = [
                    full_path,
                    "/scratch/g/g260190/tas/DJF.nc",
                    "/scratch/g/g260190/tas/MAM.nc",
                    "/scratch/g/g260190/tas/JJA.nc",
                    "/scratch/g/g260190/tas/SON.nc",
                ]
